...
let customAreaMode = false;
let customAreaCircle = null;
let customAreaRadius = 0;
let customAreaCenter = null;
// Data loading state
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeMap();
    // Initialize optional satellite overlays (NASA GIBS, etc.)
    // These are added as toggleable overlay layers and do NOT replace existing markers
    // You can safely remove this call if you don't want satellite layers
    initializeSatelliteLayers();
    setupEventListeners();
...
// Satellite overlays (NASA/ESA/CSA/INPE) using WMS endpoints
// - This function adds toggleable overlay layers that sit ABOVE the base OSM map
// - Beehive markers, plant regions, and other features remain intact
// - You can extend/replace endpoints below with additional WMS sources easily
function initializeSatelliteLayers() {
    if (!window.map) return;

    // ---------------------------------------------------------------------
    // NASA GIBS (Global Imagery Browse Services) - WMS endpoint (EPSG:3857)
    // Reference: https://gibs.earthdata.nasa.gov 
    // Layer list: https://gibs.earthdata.nasa.gov/wms/epsg3857/best/wms.cgi?request=GetCapabilities
    // ---------------------------------------------------------------------
    const gibsWms = "https://gibs.earthdata.nasa.gov/wms/epsg3857/best/wms.cgi";
    // Daily layers (MODIS/VIIRS) require a date. Use today's date by default.
    ...
    // (Removed MODIS/VIIRS from control per request; keeping Landsat Annual Mosaic only)

    // Landsat (annual global mosaic) - useful for larger-scale analysis
    const landsatTrue = L.tileLayer.wms(gibsWms, {
        layers: "Landsat_WELD_CorrectedReflectance_TrueColor_Global_Annual",
        format: "image/png",
        transparent: true,
        opacity: 0.7,
        attribution: "© NASA EOSDIS / Landsat",
        tiled: true,
        version: '1.3.0',
        tileSize: 256
    });
    ...
    // ---------------------------------------------------------------------
    // ESA Sentinel-2 (Placeholder) - requires public WMS or Sentinel Hub account
    // To enable later, replace YOUR_SENTINEL_WMS_ENDPOINT and layer name
    // Example (Sentinel Hub): https://services.sentinel-hub.com/ogc/wms/<INSTANCE_ID>
    // const sentinel2True = L.tileLayer.wms("YOUR_SENTINEL_WMS_ENDPOINT", {
    //     layers: "TRUE_COLOR",
    //     format: "image/png",
    //     transparent: true,
    //     opacity: 0.7,
    ...
    // CSA RADARSAT (Placeholder) - add official WMS endpoint when available
    // const csaRadarsat = L.tileLayer.wms("CSA_RADARSAT_WMS_ENDPOINT", {
    //     layers: "RADARSAT_LAYER",
    //     format: "image/png",
    //     transparent: true,
    //     opacity: 0.7,
    //     attribution: "© Canadian Space Agency / RADARSAT"
    // });

    // INPE (Brazil) - CBERS / Amazonia-1 (Placeholder)
    // const inpeCbers = L.tileLayer.wms("INPE_WMS_ENDPOINT", {
    //     layers: "CBERS_LAYER",
    //     format: "image/png",
    //     transparent: true,
    //     opacity: 0.7,
    ...
    // Build overlay dictionary for Leaflet's layer control
    const satelliteOverlays = {
        "NASA Landsat (Annual Mosaic)": landsatTrue
    };

    // Add an empty base layer object so these appear as overlays
    // This ensures they do not replace the existing OSM base map or markers
    const layersControl = L.control.layers(null, satelliteOverlays, { position: 'topright', collapsed: true }).addTo(map);

    // Move the layers control under the "Select Beehive Location" button
    // so it sits inside the existing map UI controls panel instead of a map corner
    // This keeps UX consistent and avoids covering the map.
    setTimeout(() => {
        const hiveControls = document.querySelector('.hive-controls');
        if (hiveControls && layersControl && layersControl._container) {
            // Create a wrapper so we can style spacing independently
            const wrapper = document.createElement('div');
            wrapper.id = 'satelliteLayerWrapper';
            wrapper.style.marginTop = '8px';
            wrapper.style.display = 'block';
            
            // Tweak Leaflet control styles for inline embedding
            const c = layersControl._container;
            c.style.position = 'relative';
            c.style.margin = '0';
            c.style.float = 'none';
            // Keep leaflet-control class for proper styling/behavior
            
            wrapper.appendChild(c);
            // Insert right after the Select Beehive button
            const selectBtn = document.getElementById('selectHiveBtn');
            if (selectBtn && selectBtn.parentElement === hiveControls) {
                // Place directly after the button element
                selectBtn.insertAdjacentElement('afterend', wrapper);
            } else {
                // Fallback: append to the controls panel
                hiveControls.appendChild(wrapper);
            }

            // Note element (hidden by default). It will show when Landsat layer is enabled
            let note = document.getElementById('satelliteLayerNote');
            if (!note) {
                note = document.createElement('div');
                note.id = 'satelliteLayerNote';
                note.style.marginTop = '6px';
                note.style.padding = '8px 10px';
                note.style.borderRadius = '6px';
                note.style.background = 'rgba(0,0,0,0.35)';
                note.style.color = '#fff';
                note.style.fontSize = '0.85rem';
                note.style.lineHeight = '1.3';
                note.style.display = 'none';
                note.style.maxWidth = '460px';
                note.style.whiteSpace = 'normal';
                note.style.wordBreak = 'break-word';
                note.innerHTML = '<strong>NASA Landsat Annual Mosaic</strong>: Annual true‑color composite highlighting long‑term land cover patterns and vegetation extent. Useful as a stable baseline for comparing seasonal flowering and beehive placement context.';
                wrapper.appendChild(note);
            }

            // Toggle note visibility when Landsat layer is shown/hidden
            landsatTrue.on('add', () => { const n = document.getElementById('satelliteLayerNote'); if (n) n.style.display = 'block'; });
            landsatTrue.on('remove', () => { const n = document.getElementById('satelliteLayerNote'); if (n) n.style.display = 'none'; });
        }
    }, 0);

    // Layers are NOT added by default; users can toggle them on-demand
    // If you want a default, uncomment one line below:
    // modisTerraTrue.addTo(map);

    console.log('Satellite overlay layers initialized');
}
...
function initializePhenologyDashboard() {
    ...
    const trendsYear = document.getElementById('trendsYear');
    ...
    if (trendsYear && !trendsYear.dataset.listenerAdded) {
        trendsYear.addEventListener('change', updateTrendsChart);
        trendsYear.dataset.listenerAdded = 'true';
    }
...
function updateTrendsChart() {
    ...
    console.log('Trends update requested:', { year, dataSource, speciesId: speciesIdStr });
    ...
    phenologyCharts.trends = new Chart(chartCtx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: `NASA ${dataSource.toUpperCase()} Data - Multi-Year Trends (${year})`,
                    color: '#fff'
                },
                legend: {
                    labels: {
...
function toggleHiveSelection() {
    ...
    if (hiveSelectionMode) {
        ...
        document.getElementById('hiveStatus').textContent = 'Click on the map to place your beehive';
...
function showHiveNameModal() {
    ...
    const radiusInput = document.getElementById('hiveRadiusInput');
    
    // Set default values
    nameInput.value = `Beehive #${nextHiveId}`;
    beeCountInput.value = 10;
    radiusInput.value = 5; // Default 5 km radius
...
function confirmHivePlacement() {
    ...
    if (radius <= 0 || radius > 50) {
        alert('Please enter a valid radius between 0.1 and 50 kilometers.');
        return;
    }
    ...
    const hive = {
        ...
        beeCount: beeCount,
        radius: radius, // Store the radius from input
        marker: null,
        circle: null,
        data: null
    };
...
function placeHive(hive) {
    ...
    const radiusKm = hive.radius || hiveRadiusKm;
    ...
    marker.bindPopup(`<strong>${hive.name}</strong><br>Colonies: ${hive.beeCount}<br>Foraging radius: ${radiusKm} km`);
    hive.marker = marker;
...
function analyzeHive(hive) {
    ...
    // Use the radius from the hive object or default
    const radiusKm = hive.radius || hiveRadiusKm;
    ...
    // Calculate honey yield per species (realistic calculation)
    const honeyYield = {};
    const monthlyYield = Array(12).fill(0);
    let totalAnnualYield = 0;
    
    // Calculate area size for realistic yield
    const areaKm2 = Math.PI * radiusKm * radiusKm;
    const flowerDensity = flowersInRadius.length / areaKm2;
    ...
    Object.keys(speciesCount).forEach(speciesName => {
        ...
        // Realistic honey per plant region per colony per year (kg)
        // Based on actual beekeeping data: 5-15 kg per colony per year
        let honeyPerRegionPerColony = 8; // Base yield
        if (honeyScore >= 8) {
            honeyPerRegionPerColony = 12; // High quality flowers
        } else if (honeyScore >= 5) {
            honeyPerRegionPerColony = 8; // Medium quality flowers
        } else {
            honeyPerRegionPerColony = 5; // Low quality flowers
        }
        
        // Apply density multiplier (fewer flowers = less efficiency)
        let densityMultiplier = 1;
        if (flowerDensity > 50) {
            densityMultiplier = 1.2; // High density
        } else if (flowerDensity > 20) {
            densityMultiplier = 1.0; // Medium density
        } else if (flowerDensity > 10) {
            densityMultiplier = 0.8; // Low density
        } else {
            densityMultiplier = 0.6; // Very low density
        }
        
        // Apply area size multiplier (smaller areas are less efficient)
        let areaMultiplier = 1;
        if (areaKm2 < 1) {
            areaMultiplier = 0.6; // Very small area
        } else if (areaKm2 < 5) {
            areaMultiplier = 0.8; // Small area
        } else if (areaKm2 < 20) {
            areaMultiplier = 1.0; // Medium area
        } else {
            areaMultiplier = 1.1; // Large area
        }
        
        // Calculate final yield per region
        const honeyPerRegion = honeyPerRegionPerColony * densityMultiplier * areaMultiplier;
...
// Custom Area Selection for Beehive Placement
function startCustomAreaSelection() {
    // Prompt user for radius in km
    const radiusInput = prompt('Enter the radius of your area in kilometers (e.g., 5):', '5');
    ...
    showCustomAreaInstruction(radius);
    
    // Wait for user to click on map
    map.once('click', function(e) {
        customAreaCenter = e.latlng;
        createCustomAreaCircle(e.latlng, radius);
        promptForBeehiveDetails(e.latlng, radius);
    });
}

function showCustomAreaInstruction(radius) {
    const notification = document.createElement('div');
    notification.style.cssText = `
        position: fixed;
        top: 50%;
    ...
    notification.innerHTML = `
        <div style="display: flex; align-items: center; justify-content: center; gap: 15px; margin-bottom: 15px;">
            <i class="fas fa-map-marker-alt" style="font-size: 2rem; color: #f39c12;"></i>
            <span style="font-size: 1.4rem;">Select Area Center</span>
        </div>
        <div style="font-size: 1.1rem; opacity: 0.9; line-height: 1.5;">
            Click on the map to place the center of your ${radius} km radius area.
        </div>
        <div style="margin-top: 15px; font-size: 0.9rem; opacity: 0.7;">
            This will analyze flowers and calculate honey yield for your selected area.
        </div>
    `;
    ...
    setTimeout(() => {
        if (notification.parentNode) {
            notification.style.animation = 'fadeOutScale 0.6s ease';
            setTimeout(() => {
                if (notification.parentNode) {
                    notification.parentNode.removeChild(notification);
                }
...
function createCustomAreaCircle(center, radius) {
    // Remove existing custom area circle if any
    if (customAreaCircle) {
        map.removeLayer(customAreaCircle);
    }
    
    // Create new circle
    customAreaCircle = L.circle(center, {
...
function promptForBeehiveDetails(center, radius) {
    // Calculate area in square kilometers
    const areaKm2 = Math.PI * radius * radius;
    
    // Analyze the area for flowers
    const areaData = analyzeCustomArea(center, radius);
    
    // Calculate recommended hive count based on area and flower density
    const recommendedHives = calculateRecommendedHiveCount(areaKm2, areaData);
    
    // Show modal with details and recommendations
    showCustomAreaModal(center, radius, areaKm2, areaData, recommendedHives);
}

function analyzeCustomArea(center, radiusKm) {
    const data = {
        speciesData: {},
        speciesDistribution: {},
        totalFlowers: 0,
        honeyPotential: 0,
        seasonalDistribution: Array(12).fill(0)
    };
    
    const radiusMeters = radiusKm * 1000;
    
    // Analyze plant markers within the radius
    plantMarkers.forEach(marker => {
        const markerLatLng = marker.getLatLng();
        const distance = map.distance(center, markerLatLng);
        
        if (distance <= radiusMeters) {
            const plant = marker.plant;
            if (plant) {
                // Add to species data
                if (!data.speciesData[plant.name]) {
                    data.speciesData[plant.name] = plant;
                    data.speciesDistribution[plant.name] = 0;
                }
                data.speciesDistribution[plant.name]++;
                data.totalFlowers++;
                
                // Add honey potential
                const honeyScore = plant.characteristics?.honeyScore || 5;
                data.honeyPotential += honeyScore;
                
                // Add to seasonal distribution
                if (plant.phenology?.peakBloomMonth !== undefined) {
                    const month = plant.phenology.peakBloomMonth;
                    if (month >= 0 && month < 12) {
                        data.seasonalDistribution[month]++;
                    }
                }
            }
        }
...
function calculateRecommendedHiveCount(areaKm2, areaData) {
    // Base calculation: 1 hive per 2 km² for good flower density
    // Adjust based on flower count and honey potential
    
    const baseHives = Math.ceil(areaKm2 / 2);
    
    // Adjust based on flower density
    const flowerDensity = areaData.totalFlowers / areaKm2;
    let densityMultiplier = 1;
    
    if (flowerDensity > 100) {
        densityMultiplier = 1.5; // High density: can support more hives
    } else if (flowerDensity < 50) {
        densityMultiplier = 0.7; // Low density: fewer hives recommended
    }
    
    // Adjust based on honey potential
    const avgHoneyScore = areaData.totalFlowers > 0 ? 
        areaData.honeyPotential / areaData.totalFlowers : 5;
    
    let honeyMultiplier = 1;
    if (avgHoneyScore > 7) {
        honeyMultiplier = 1.2; // High honey potential
    } else if (avgHoneyScore < 4) {
        honeyMultiplier = 0.8; // Low honey potential
    }
    
    const recommendedCount = Math.max(1, Math.round(baseHives * densityMultiplier * honeyMultiplier));
    
    return {
        min: Math.max(1, recommendedCount - 2),
        optimal: recommendedCount,
        max: recommendedCount + 3,
        reasoning: {
            baseHives,
            flowerDensity: flowerDensity.toFixed(1),
            avgHoneyScore: avgHoneyScore.toFixed(1),
            densityMultiplier,
            honeyMultiplier
        }
    };
...
function showCustomAreaModal(center, radius, areaKm2, areaData, recommendedHives) {
    // Calculate estimated honey yield
    const estimatedYield = calculateHoneyYield(areaData, recommendedHives.optimal);
    ...
    modalContent.innerHTML = `
        ...
        <div style="background: rgba(255, 255, 255, 0.1); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <h3 style="margin-top: 0; color: #3498db;">
                <i class="fas fa-map-marked-alt"></i> Area Details
            </h3>
            <p><strong>Radius:</strong> ${radius} km</p>
            <p><strong>Total Area:</strong> ${areaKm2.toFixed(2)} km²</p>
            <p><strong>Flower Regions:</strong> ${areaData.totalFlowers}</p>
            <p><strong>Plant Species:</strong> ${Object.keys(areaData.speciesData).length}</p>
            <p><strong>Flower Density:</strong> ${(areaData.totalFlowers / areaKm2).toFixed(1)} regions/km²</p>
        </div>
        
        <div style="background: rgba(243, 156, 18, 0.2); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <h3 style="margin-top: 0; color: #f39c12;">
                <i class="fas fa-hive"></i> Recommended Hive Count
            </h3>
            <p style="font-size: 1.1rem;">
                <strong>Optimal:</strong> <span style="font-size: 1.5rem; color: #f39c12;">${recommendedHives.optimal}</span> hives
            </p>
            <p><strong>Range:</strong> ${recommendedHives.min} - ${recommendedHives.max} hives</p>
            <p style="font-size: 0.9rem; opacity: 0.8;">
                Based on area size (${areaKm2.toFixed(2)} km²), flower density (${recommendedHives.reasoning.flowerDensity} regions/km²), 
                and honey potential (score: ${recommendedHives.reasoning.avgHoneyScore})
            </p>
        </div>
        
        <div style="background: rgba(46, 204, 113, 0.2); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <h3 style="margin-top: 0; color: #2ecc71;">
                <i class="fas fa-honey-pot"></i> Estimated Honey Yield
            </h3>
            <p style="font-size: 1.1rem;">
                <strong>Annual Yield:</strong> <span style="font-size: 1.5rem; color: #f39c12;">${estimatedYield.total.toFixed(1)}</span> kg
            </p>
            <p><strong>Per Hive:</strong> ${estimatedYield.perHive.toFixed(1)} kg/year</p>
            <p><strong>Peak Season:</strong> ${estimatedYield.peakSeason}</p>
            <div style="background: rgba(0, 0, 0, 0.2); padding: 15px; border-radius: 8px; margin-top: 15px;">
                <h4 style="margin-top: 0; color: #3498db; font-size: 1rem;">Calculation Factors:</h4>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 0.9rem;">
                    <div><strong>Base Yield:</strong> ${estimatedYield.factors.baseYield} kg/hive</div>
                    <div><strong>Area Size:</strong> ${estimatedYield.factors.areaKm2} km²</div>
                    <div><strong>Flower Density:</strong> ${estimatedYield.factors.flowerDensity} regions/km²</div>
                    <div><strong>Avg Honey Score:</strong> ${estimatedYield.factors.avgHoneyScore}/10</div>
                    <div><strong>Density Multiplier:</strong> ${estimatedYield.factors.densityMultiplier}x</div>
                    <div><strong>Honey Multiplier:</strong> ${estimatedYield.factors.honeyMultiplier}x</div>
                    <div><strong>Area Multiplier:</strong> ${estimatedYield.factors.areaMultiplier}x</div>
                    <div><strong>Final Formula:</strong> ${estimatedYield.factors.baseYield} × ${estimatedYield.factors.densityMultiplier} × ${estimatedYield.factors.honeyMultiplier} × ${estimatedYield.factors.areaMultiplier}</div>
                </div>
            </div>
            <p style="font-size: 0.9rem; opacity: 0.8; margin-top: 15px;">
                Estimate based on ${recommendedHives.optimal} hives and realistic beekeeping data
            </p>
        </div>
        
        <div style="background: rgba(52, 152, 219, 0.2); padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <h3 style="margin-top: 0; color: #3498db;">
                <i class="fas fa-seedling"></i> Top Flower Species
            </h3>
            ${getTopSpeciesHTML(areaData)}
        </div>
        
        <div style="display: flex; gap: 10px; margin-top: 25px;">
            <input type="text" id="customHiveName" placeholder="Enter hive name" 
                   style="flex: 1; padding: 12px; border-radius: 8px; border: none; font-size: 1rem;"
                   value="Hive Area ${nextHiveId}">
            <input type="number" id="customHiveCount" placeholder="Hive count" 
                   style="width: 120px; padding: 12px; border-radius: 8px; border: none; font-size: 1rem;"
                   value="${recommendedHives.optimal}" min="1">
        </div>
        
        <div style="display: flex; gap: 10px; margin-top: 15px;">
            <button onclick="confirmCustomArea()" 
                    style="flex: 1; padding: 12px 24px; background: #2ecc71; color: white; border: none; 
                           border-radius: 8px; font-size: 1.1rem; font-weight: 600; cursor: pointer;">
                <i class="fas fa-check"></i> Create Hive Area
            </button>
            <button onclick="cancelCustomArea()" 
                    style="padding: 12px 24px; background: #e74c3c; color: white; border: none; 
...
function getTopSpeciesHTML(areaData) {
    const sorted = Object.entries(areaData.speciesDistribution)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5);
    
    if (sorted.length === 0) {
        return '<p style="opacity: 0.7;">No flower species found in this area.</p>';
    }
    
    let html = '<div style="display: flex; flex-direction: column; gap: 10px;">';
    sorted.forEach(([name, count]) => {
        const plant = areaData.speciesData[name];
        const honeyScore = plant.characteristics?.honeyScore || 5;
        html += `
            <div style="display: flex; justify-content: space-between; align-items: center; 
                ...
                <span style="color: #f39c12;">
                    ${count} regions | Honey: ${honeyScore}/10
                </span>
            </div>
        `;
    });
...
function calculateHoneyYield(areaData, hiveCount) {
    // Realistic honey yield calculation based on actual data
    
    // Base yield per hive per year: 10-20 kg (realistic range)
    const baseYieldPerHive = 15; // kg per year
    
    // Calculate flower density per km²
    const areaKm2 = Math.PI * (hiveRadiusKm || 5) * (hiveRadiusKm || 5);
    const flowerDensity = areaData.totalFlowers / areaKm2;
    
    // Calculate average honey score
    const avgHoneyScore = areaData.totalFlowers > 0 ? 
        areaData.honeyPotential / areaData.totalFlowers : 5;
    
    // Realistic multipliers based on flower density
    let densityMultiplier = 1;
    if (flowerDensity > 50) {
        densityMultiplier = 1.3; // High density
    } else if (flowerDensity > 20) {
        densityMultiplier = 1.1; // Medium density
    } else if (flowerDensity > 10) {
        densityMultiplier = 0.9; // Low density
    } else {
        densityMultiplier = 0.7; // Very low density
    }
    
    // Honey score multiplier (more conservative)
    const honeyMultiplier = Math.min(avgHoneyScore / 5, 1.5); // Cap at 1.5x
    
    // Area size multiplier (smaller areas are less efficient)
    let areaMultiplier = 1;
    if (areaKm2 < 1) {
        areaMultiplier = 0.6; // Very small area
    } else if (areaKm2 < 5) {
        ...
        areaMultiplier = 1.2; // Large area
    }
    
    // Calculate final yield
    const perHiveYield = baseYieldPerHive * densityMultiplier * honeyMultiplier * areaMultiplier;
    const totalYield = perHiveYield * hiveCount;
    
    // Determine peak season
    const peakMonth = areaData.seasonalDistribution.indexOf(
        Math.max(...areaData.seasonalDistribution)
    );
    const months = ['January', 'February', 'March', 'April', 'May', 'June', 
                    'July', 'August', 'September', 'October', 'November', 'December'];
    
    return {
        total: Math.round(totalYield * 10) / 10, // Round to 1 decimal
        perHive: Math.round(perHiveYield * 10) / 10,
        peakSeason: months[peakMonth] || 'Summer',
        factors: {
            baseYield: baseYieldPerHive,
            flowerDensity: flowerDensity.toFixed(1),
            avgHoneyScore: avgHoneyScore.toFixed(1),
            densityMultiplier: densityMultiplier.toFixed(2),
            honeyMultiplier: honeyMultiplier.toFixed(2),
            areaMultiplier: areaMultiplier.toFixed(2),
            areaKm2: areaKm2.toFixed(2)
        }
    };
}

function confirmCustomArea() {
    const data = window.tempCustomAreaData;
    if (!data) return;
    
    const hiveName = document.getElementById('customHiveName').value || `Hive Area ${nextHiveId}`;
    const hiveCount = parseInt(document.getElementById('customHiveCount').value) || 1;
        ...
        icon: L.divIcon({
            html: `
                <div style="background: linear-gradient(135deg, #f39c12, #e67e22); 
                    ...
                    <i class="fas fa-hive"></i> ${hiveName}
                </div>
            `,
            className: '',
            iconSize: [150, 40],
    ...
    marker.bindPopup(`
        <strong>${hiveName}</strong><br>
        Hives: ${hiveCount}<br>
        Area: ${(Math.PI * data.radius * data.radius).toFixed(2)} km²<br>
        Flowers: ${data.areaData.totalFlowers} regions
    `);
    ...
    // Close modal
    data.modal.remove();
    window.tempCustomAreaData = null;
    customAreaCircle = null;
    
    // Switch to harvesting tab and show results
    switchTab('harvesting');
    updateHarvestingDashboard();
    
    // Show success message
    showSuccessMessage(`${hiveName} created successfully with ${hiveCount} hives!`);
}
